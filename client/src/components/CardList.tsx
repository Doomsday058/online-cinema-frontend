...
const CardList: React.FC<CardListProps> = ({ type }) => {
    ...
    try {
      ...
      const enrichedFavorites = await Promise.all(
        favorites.map(async (favorite: { tmdbId: number; type: string }) => {
          ...
          const tmdbResponse = await fetch(`http://localhost:8000/api/${endpoint}/${favorite.tmdbId}`);
  ...
  const fetchPopular = async (pageNumber: number) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/${endpoint}?page=${pageNumber}`);
  ...
  const removeFromFavorites = async (tmdbId: number, itemType: 'movie' | 'serial') => {
    try {
      const response = await fetch(`http://localhost:8000/api/favorites/${userId}/${tmdbId}`, {
        method: 'DELETE',
      });
