...
const Card: React.FC<CardProps> = ({ id, title, poster, rating, type, onRemove }) => {
  ...
  const [animating, setAnimating] = useState(false); // Состояние для анимации
  ...
  const handleFavoriteClick = async (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    try {
      if (isFavorite) {
        await fetch(`http://localhost:8000/api/favorites/${userId}/${id}`, { method: 'DELETE' });
  ...
  const triggerAnimation = () => {
    setAnimating(true);
    setTimeout(() => setAnimating(false), 300); // Длительность анимации
  };

  // Форматирование рейтинга до одной цифры после точки
